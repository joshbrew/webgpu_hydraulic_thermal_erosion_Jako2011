...
\`;var gpu=null;var offscreenCanvas=null;var canvasMetrics={width:1,height:1,dpr:1};var running=false;var loopHandle=0;var lastFrameAt=0;var lastFrameMs=0;var lastStatusPostAt=0;var lastRenderAt=0;var lastGpuBenchAt=0;var loopIterationsPerFrame=5;var simulationParams={};var sourceRaster=null;var sourceImageInfo=null;var gpuInitPromise=null;var hasWorkerRAF=typeof self.requestAnimationFrame==="function";var workerRAF=hasWorkerRAF?cb=>self.requestAnimationFrame(cb):cb=>self.setTimeout(()=>cb(performance.now()),0);var workerCAF=hasWorkerRAF?id=>self.cancelAnimationFrame(id):id=>self.clearTimeout(id);function cloneStats(stats){if(!stats)return null;return{...stats,terrainRange:stats.terrainRange?{...stats.terrainRange}:null,waterRange:stats.waterRange?{...stats.waterRange}:null,sedimentRange:stats.sedimentRange?{...stats.sedimentRange}:null,historyRange:stats.historyRange?{...stats.historyRange}:null}}function getSourcePoints(){return gpu?.getSourcePoints?.()||[]}function postStatus(force=false){const stats=gpu?cloneStats(gpu.getStats()):null;self.postMessage({type:"status",stats,sourcePoints:force?getSourcePoints():void 0,lastFrameMs,running,gpuReady:!!gpu,simReady:!!stats?.ready,sourceImageInfo:force?sourceImageInfo:void 0})}function applyCanvasMetrics(){if(!offscreenCanvas)return;const width=Math.max(1,Math.round((canvasMetrics.width||1)*(canvasMetrics.dpr||1)));const height=Math.max(1,Math.round((canvasMetrics.height||1)*(canvasMetrics.dpr||1)));offscreenCanvas.width=width;offscreenCanvas.height=height;if(gpu){gpu.resize()}}async function ensureGpu(){if(!offscreenCanvas){throw new Error("Worker canvas has not been initialized.")}if(gpu){return gpu}if(!gpuInitPromise){gpuInitPromise=(async()=>{const instance=new WebGPUTerrainErosion({canvas:offscreenCanvas});await instance.initialize();if(Object.keys(simulationParams).length>0){instance.setSimulationParams(simulationParams)}gpu=instance;applyCanvasMetrics();return instance})().catch(error=>{gpu=null;gpuInitPromise=null;throw error})}try{return await gpuInitPromise}finally{if(gpu){gpuInitPromise=null}}}function warmGpuInBackground(){if(gpu||gpuInitPromise||!offscreenCanvas)return;void ensureGpu().then(()=>{postStatus(true)}).catch(error=>{self.postMessage({type:"workerError",message:error instanceof Error?error.message:String(error)})})}function stopLoopInternal(){if(!running)return;running=false;if(loopHandle){workerCAF(loopHandle);loopHandle=0}}function scheduleNextFrame(){loopHandle=workerRAF(now=>{void frame(now)})}function getLoopRenderIntervalMs(){const iterations=Math.max(1,loopIterationsPerFrame|0);if(iterations<=4)return 0;if(iterations<=8)return 16;if(iterations<=16)return 33;return 50}async function frame(now){if(!running)return;const currentGpu=gpu;if(!currentGpu?.ready){scheduleNextFrame();return}lastFrameMs=lastFrameAt>0?Math.max(0,now-lastFrameAt):0;lastFrameAt=now;try{const iterations=Math.max(1,loopIterationsPerFrame|0);const renderIntervalMs=getLoopRenderIntervalMs();const shouldRender=renderIntervalMs<=0||lastRenderAt<=0||now-lastRenderAt>=renderIntervalMs;const shouldBenchmarkGpu=lastGpuBenchAt<=0||now-lastGpuBenchAt>=1e3;if(shouldRender){if(shouldBenchmarkGpu){await currentGpu.stepAndRenderBench(iterations);lastGpuBenchAt=now}else{currentGpu.stepAndRender(iterations)}lastRenderAt=now}else if(shouldBenchmarkGpu){await currentGpu.stepBench(iterations);lastGpuBenchAt=now}else{currentGpu.step(iterations)}if(now-lastStatusPostAt>=1e3){lastStatusPostAt=now;postStatus()}}catch(error){stopLoopInternal();self.postMessage({type:"workerError",message:error instanceof Error?error.message:String(error)});return}scheduleNextFrame()}function reply(requestId,payload={},transfer=[]){if(!requestId)return;self.postMessage({type:"response",requestId,ok:true,...payload},transfer)}function replyError(requestId,error){if(!requestId)return;self.postMessage({type:"response",requestId,ok:false,error:error instanceof Error?error.message:String(error)})}async function buildRasterFromBlob(blob,mode="single"){if(!(blob instanceof Blob)){throw new Error("DEM image payload must be a Blob or File.")}if(typeof createImageBitmap!=="function"){throw new Error("createImageBitmap is not available in this worker.")}const bitmap=await createImageBitmap(blob,{imageOrientation:"none"});try{return await buildRasterFromBitmap(bitmap,mode)}finally{bitmap.close?.()}}async function buildRasterFromBitmap(bitmap,mode="single"){const width=bitmap.width|0;const height=bitmap.height|0;const canvas=new OffscreenCanvas(width,height);const ctx=canvas.getContext("2d",{willReadFrequently:true});if(!ctx){throw new Error("Failed to acquire worker raster canvas context.")}ctx.setTransform(1,0,0,-1,0,height);ctx.clearRect(0,0,width,height);ctx.drawImage(bitmap,0,0,width,height);ctx.setTransform(1,0,0,1,0,0);const imageData=ctx.getImageData(0,0,width,height);const rgba=imageData.data;const count=width*height;const packedRgba=mode==="packed_rgba";const mask=new Uint8Array(count);if(packedRgba){const bands=[new Float32Array(count),new Float32Array(count),new Float32Array(count),new Float32Array(count)];const values2=bands[0];for(let i=0,j=0;i<count;i++,j+=4){values2[i]=rgba[j]/255;bands[1][i]=rgba[j+1]/255;bands[2][i]=rgba[j+2]/255;bands[3][i]=rgba[j+3]/255;mask[i]=1}return{width,height,values:values2,mask,bands}}const values=new Float32Array(count);for(let i=0,j=0;i<count;i++,j+=4){values[i]=(rgba[j]+rgba[j+1]+rgba[j+2])/(3*255);mask[i]=rgba[j+3]===0?0:1}return{width,height,values,mask,bands:null}}async function buildStackedLayerRasterFromBlobs(layerBlobs=[]){const firstIndex=layerBlobs.findIndex(Boolean);if(firstIndex<0){throw new Error("Layer stack mode requires at least one layer PNG.")}const first=await buildRasterFromBlob(layerBlobs[firstIndex],"single");const width=first.width|0;const height=first.height|0;const count=width*height;const bands=[new Float32Array(count),new Float32Array(count),new Float32Array(count),new Float32Array(count)];const values=bands[0];const mask=first.mask instanceof Uint8Array&&first.mask.length===count?first.mask.slice(0):new Uint8Array(count).fill(1);bands[firstIndex].set(first.values);releaseRaster(first);for(let layerIndex=0;layerIndex<4;layerIndex++){if(layerIndex===firstIndex||!layerBlobs[layerIndex])continue;const layer=await buildRasterFromBlob(layerBlobs[layerIndex],"single");if(layer.width!==width||layer.height!==height){throw new Error("All layer PNGs in stack mode must have the same dimensions.")}bands[layerIndex].set(layer.values);if(layer.mask instanceof Uint8Array&&layer.mask.length===count){for(let i=0;i<count;i++){mask[i]=mask[i]&&layer.mask[i]?1:0}}releaseRaster(layer)}return{width,height,values,mask,bands}}function resampleRasterBilinear(raster,scale){const safeScale=Math.max(1,Math.floor(Number(scale)||1));if(!raster||safeScale===1)return raster;const srcWidth=raster.width|0;const srcHeight=raster.height|0;const dstWidth=Math.max(1,(srcWidth-1)*safeScale+1);const dstHeight=Math.max(1,(srcHeight-1)*safeScale+1);const dstMask=new Uint8Array(dstWidth*dstHeight);const srcValues=raster.values;const srcMask=raster.mask instanceof Uint8Array?raster.mask:null;const srcBands=Array.isArray(raster.bands)?raster.bands:null;const dstBands=srcBands?srcBands.map(()=>new Float32Array(dstWidth*dstHeight)):null;const dstValues=dstBands?dstBands[0]:new Float32Array(dstWidth*dstHeight);function sampleArray(srcArray,px,py){const x0=Math.floor(px);const y0=Math.floor(py);const x1=Math.min(x0+1,srcWidth-1);const y1=Math.min(y0+1,srcHeight-1);const tx=px-x0;const ty=py-y0;const i00=y0*srcWidth+x0;const i10=y0*srcWidth+x1;const i01=y1*srcWidth+x0;const i11=y1*srcWidth+x1;const v00=srcArray[i00];const v10=srcArray[i10];const v01=srcArray[i01];const v11=srcArray[i11];const a=v00+(v10-v00)*tx;const b=v01+(v11-v01)*tx;return a+(b-a)*ty}function sampleMask(px,py){if(!srcMask)return 1;const x0=Math.floor(px);const y0=Math.floor(py);const x1=Math.min(x0+1,srcWidth-1);const y1=Math.min(y0+1,srcHeight-1);const tx=px-x0;const ty=py-y0;const i00=y0*srcWidth+x0;const i10=y0*srcWidth+x1;const i01=y1*srcWidth+x0;const i11=y1*srcWidth+x1;const m00=srcMask[i00];const m10=srcMask[i10];const m01=srcMask[i01];const m11=srcMask[i11];const a=m00+(m10-m00)*tx;const b=m01+(m11-m01)*tx;return a+(b-a)*ty>=.5?1:0}for(let y=0;y<dstHeight;y++){const srcY=y/safeScale;for(let x=0;x<dstWidth;x++){const srcX=x/safeScale;const dstIndex=y*dstWidth+x;dstValues[dstIndex]=sampleArray(srcValues,srcX,srcY);dstMask[dstIndex]=sampleMask(srcX,srcY);if(dstBands&&srcBands){for(let bandIndex=1;bandIndex<dstBands.length;bandIndex++){dstBands[bandIndex][dstIndex]=sampleArray(srcBands[bandIndex],srcX,srcY)}}}}return{width:dstWidth,height:dstHeight,values:dstValues,mask:dstMask,bands:dstBands}}function releaseRaster(raster){if(!raster)return;raster.values=null;raster.mask=null;raster.bands=null}async function waitForGpuCompletion(currentGpu){const queue=currentGpu?.device?.queue;if(!queue||typeof queue.onSubmittedWorkDone!=="function")return;await queue.onSubmittedWorkDone()}async function loadRasterIntoGpu(message){const currentGpu=await ensureGpu();if(Object.keys(simulationParams).length>0){currentGpu.setSimulationParams(simulationParams)}const demSourceMode=message.demSourceMode||message.options?.demSourceMode||"single";if(Array.isArray(message.layerBlobs)&&message.layerBlobs.some(Boolean)){sourceRaster=await buildStackedLayerRasterFromBlobs(message.layerBlobs);sourceImageInfo={width:sourceRaster.width,height:sourceRaster.height,mode:"stack4"}}else if(message.blob){sourceRaster=await buildRasterFromBlob(message.blob,demSourceMode);sourceImageInfo={width:sourceRaster.width,height:sourceRaster.height,mode:demSourceMode}}if(!sourceRaster){throw new Error("No DEM image has been loaded into the worker yet.")}const tessellation=Math.max(1,Math.floor(Number(message.tessellation)||1));const raster=resampleRasterBilinear(sourceRaster,tessellation);await currentGpu.setDEM(raster,{...message.options||{},demSourceMode});currentGpu.render();const stats=cloneStats(currentGpu.getStats());if(raster!==sourceRaster){releaseRaster(raster)}return{stats,sourcePoints:getSourcePoints(),ready:true,sourceImageInfo,simSize:{width:raster.width,height:raster.height}}}self.onmessage=async event=>{const message=event.data||{};const{type,requestId}=message;try{switch(type){case"init":{offscreenCanvas=message.canvas;canvasMetrics={width:Math.max(1,Number(message.width)||1),height:Math.max(1,Number(message.height)||1),dpr:Math.max(1,Number(message.dpr)||1)};applyCanvasMetrics();warmGpuInBackground();reply(requestId,{initialized:true,warming:true});postStatus(true);break}case"resize":{canvasMetrics={width:Math.max(1,Number(message.width)||canvasMetrics.width||1),height:Math.max(1,Number(message.height)||canvasMetrics.height||1),dpr:Math.max(1,Number(message.dpr)||canvasMetrics.dpr||1)};applyCanvasMetrics();gpu?.render?.();reply(requestId,{resized:true});break}case"setParams":{simulationParams={...simulationParams,...message.params||{}};if(gpu){gpu.setSimulationParams(simulationParams)}reply(requestId,{applied:true});break}case"loadDEMImage":{const payload=await loadRasterIntoGpu(message);reply(requestId,payload);postStatus(true);break}case"setDEM":{const currentGpu=await ensureGpu();if(Object.keys(simulationParams).length>0){currentGpu.setSimulationParams(simulationParams)}const raster=message.raster||{};await currentGpu.setDEM({width:raster.width|0,height:raster.height|0,values:raster.values instanceof Float32Array?raster.values:new Float32Array(raster.values||[]),mask:raster.mask?raster.mask instanceof Uint8Array?raster.mask:new Uint8Array(raster.mask):null,bands:Array.isArray(raster.bands)?raster.bands.map(band=>band instanceof Float32Array?band:new Float32Array(band||[])):null},{...message.options||{},demSourceMode:message.options?.demSourceMode||"single"});currentGpu.render();const stats=cloneStats(currentGpu.getStats());reply(requestId,{stats,sourcePoints:getSourcePoints(),ready:true,sourceImageInfo});postStatus(true);break}case"render":{if(gpu?.ready){gpu.render();if(message.waitForCompletion){await waitForGpuCompletion(gpu)}}reply(requestId,{rendered:true});break}case"step":{if(gpu?.ready){if(message.render!==false){await gpu.stepAndRenderBench(Math.max(1,Number(message.iterations)||1))}else{await gpu.stepBench(Math.max(1,Number(message.iterations)||1))}}reply(requestId,{stepped:true,stats:cloneStats(gpu?.getStats?.()||null),sourcePoints:getSourcePoints()});break}case"readbackStats":{const stats=cloneStats(gpu?.getStats?.()||null);reply(requestId,{stats,sourcePoints:getSourcePoints(),lastFrameMs,running,sourceImageInfo});break}case"exportTerrainPng":{if(!gpu?.ready){throw new Error("Terrain export is only available after the GPU sim is ready.")}const exportResult=await gpu.exportTerrainPng();reply(requestId,{width:exportResult.width,height:exportResult.height,minTerrain:exportResult.minTerrain,maxTerrain:exportResult.maxTerrain,data:exportResult.data},[exportResult.data]);break}case"paintTerrainBrush":{if(!gpu?.ready){throw new Error("Terrain painting is only available after the GPU sim is ready.")}const stats=cloneStats(await gpu.applyTerrainBrush(message.brush||{}));gpu.render();reply(requestId,{stats,sourcePoints:getSourcePoints()});postStatus(true);break}case"paintSpringBrush":{if(!gpu?.ready){throw new Error("Spring painting is only available after the GPU sim is ready.")}gpu.paintSpringBrush(message.brush||{});const stats=cloneStats(gpu.getStats());reply(requestId,{stats,sourcePoints:getSourcePoints()});postStatus(true);break}case"clearPaintedSprings":{if(gpu?.ready){gpu.clearPaintedSprings()}const stats=cloneStats(gpu?.getStats?.()||null);reply(requestId,{stats,sourcePoints:getSourcePoints()});postStatus(true);break}case"resetRainTimer":{if(gpu?.ready){gpu.resetRainTimer();gpu.render()}const stats=cloneStats(gpu?.getStats?.()||null);reply(requestId,{stats,sourcePoints:getSourcePoints()});break}case"restartSources":{if(gpu?.ready){gpu.resetRainTimer();gpu.render()}const stats=cloneStats(gpu?.getStats?.()||null);reply(requestId,{stats,sourcePoints:getSourcePoints()});break}case"startLoop":{if(Number.isFinite(message.iterationsPerFrame)){loopIterationsPerFrame=Math.max(1,Number(message.iterationsPerFrame)|0)}if(!running){running=true;lastFrameAt=0;lastStatusPostAt=0;lastRenderAt=0;lastGpuBenchAt=0;scheduleNextFrame()}reply(requestId,{running:true,iterationsPerFrame:loopIterationsPerFrame});break}case"stopLoop":{stopLoopInternal();if(gpu?.ready){gpu.render()}reply(requestId,{running:false});postStatus(true);break}case"clear":{stopLoopInternal();if(gpu){gpu.destroy();gpu=null}gpuInitPromise=null;sourceRaster=null;sourceImageInfo=null;reply(requestId,{cleared:true});postStatus(true);break}case"getStatus":{reply(requestId,{stats:cloneStats(gpu?.getStats?.()||null),sourcePoints:getSourcePoints(),lastFrameMs,running,sourceImageInfo});break}case"destroy":{stopLoopInternal();if(gpu){gpu.destroy();gpu=null}gpuInitPromise=null;sourceRaster=null;sourceImageInfo=null;reply(requestId,{destroyed:true});self.close();break}default:throw new Error(\`Unknown worker message type: \${String(type)}\`)}}catch(error){replyError(requestId,error)}};;if(typeof import_meta !== 'undefined')import_meta.url=location.origin+"/dist/";})();
`,er=URL.createObjectURL(new globalThis.Blob([Qa],{type:"text/javascript"})),Pa=er;var It=0,Dt=1,se=5,it=128,Ot=.02,Ut=.001,Gt=.015,Wt=20,_t=9.81,zt=.82,Nt=.32,Ht=1.28,Vt=2.8,Yt=.09,$t=.24,qt=.92,Xt=.12,Kt=.045,st=.06,Zt=0,jt=.92,Jt=.35,re=.16,ot=.03,Qt=35,ea=18,ta=4,aa=.06,lt=0,dt=0,Ma=0,ut=4,ra=1,ct=100,ht=45,pt=42;var Ba=1.5238999619464006,Ea=1.9404787584406888,Ra=1.5238999619464006,tr=1.35,ar=.14,Ca=1,rr=!0,nr=!0,ir=!0,sr=!0,na=12,ia=.03,sa=.7,Ta="single",fe={sand:{label:"sand",hardnessMin:.42,hardnessMax:.7},silt:{label:"silt",hardnessMin:.24,hardnessMax:.42},clay:{label:"clay",hardnessMin:.1,hardnessMax:.22},soft_rock:{label:"soft rock",hardnessMin:.03,hardnessMax:.1},bedrock:{label:"bedrock",hardnessMin:0,hardnessMax:.02},custom:{label:"custom",hardnessMin:.2,hardnessMax:.4},default_base:{label:"base",hardnessMin:re,hardnessMax:re+ot}},At=[{enabled:!0,preset:"default_base",heightMin:0,heightMax:1,thermalEnabled:!0},{enabled:!0,preset:"silt",heightMin:.46,heightMax:.82,thermalEnabled:!0},{enabled:!0,preset:"clay",heightMin:.2,heightMax:.58,thermalEnabled:!0},{enabled:!0,preset:"bedrock",heightMin:0,heightMax:.3,thermalEnabled:!0}],or=!1;function mt(...e){or&&console.log("[WebGPU Erosion UI]",...e)}function J(e){let t=document.createElement("button");return t.textContent=e,t.style.padding="8px 12px",t.style.border="1px solid #444",t.style.borderRadius="8px",t.style.background="#1f1f1f",t.style.color="#f0f0f0",t.style.cursor="pointer",t.addEventListener("mouseenter",()=>{t.style.background="#2a2a2a"}),t.addEventListener("mouseleave",()=>{t.style.background="#1f1f1f"}),t}function d(e,t,r="88px",i={}){let n=document.createElement("label");n.textContent=e,n.style.display="inline-flex",n.style.alignItems="center",n.style.gap="8px";let o=document.createElement("input");return o.type="number",o.value=String(t),o.min=String(i.min??-1e9),o.max=String(i.max??1e9),o.step=String(i.step??.01),o.style.width=r,o.style.padding="6px 8px",o.style.border="1px solid #444",o.style.borderRadius="6px",o.style.background="#1a1a1a",o.style.color="#e8e8e8",n.input=o,n.appendChild(o),n}function ne(e,t){let r=document.createElement("label");r.style.display="inline-flex",r.style.alignItems="center",r.style.gap="6px";let i=document.createElement("input");i.type="checkbox",i.checked=t;let n=document.createElement("span");return n.textContent=e,r.input=i,r.append(i,n),r}function lr(e){let t=document.createElement("label");t.style.display="inline-flex",t.style.alignItems="center",t.style.gap="8px",t.style.flexWrap="wrap";let r=document.createElement("span");r.textContent=e,r.style.fontSize="12px",r.style.opacity="0.9";let i=document.createElement("input");return i.type="file",i.accept=".png,image/png",i.style.padding="6px 8px",i.style.border="1px solid #444",i.style.borderRadius="6px",i.style.background="#1a1a1a",i.style.color="#e8e8e8",t.input=i,t.caption=r,t.append(r,i),t}function oe(e,t,r){let i=document.createElement("label");i.textContent=e,i.style.display="inline-flex",i.style.alignItems="center",i.style.gap="8px";let n=document.createElement("select");n.style.padding="6px 8px",n.style.border="1px solid #444",n.style.borderRadius="6px",n.style.background="#1a1a1a",n.style.color="#e8e8e8";for(let{value:o,label:l}of t){let c=document.createElement("option");c.value=String(o),c.textContent=l,o===r&&(c.selected=!0),n.appendChild(c)}return i.input=n,i.appendChild(n),i}function s(e,t){let r=Number(e.input.value);return Number.isFinite(r)?r:t}function V(e){let t=document.createElement("section");t.style.border="1px solid #2d2d2d",t.style.borderRadius="10px",t.style.padding="10px",t.style.background="#141414";let r=document.createElement("div");r.textContent=e,r.style.fontSize="13px",r.style.fontWeight="600",r.style.marginBottom="8px",r.style.letterSpacing="0.02em";let i=document.createElement("div");return i.style.display="flex",i.style.flexWrap="wrap",i.style.gap="8px",t.append(r,i),t.body=i,t.heading=r,t}function Y(e){let t=document.createElement("div");return t.textContent=e,t.style.fontSize="12px",t.style.lineHeight="1.35",t.style.color="rgba(255,255,255,0.68)",t.style.margin="-2px 0 8px 0",t}function Aa(){return Math.max(1,Number(ca.input.value)||Ca)}function rt(e,t=4){return!e||!Number.isFinite(e.min)||!Number.isFinite(e.max)?"n/a":`${e.min.toFixed(t)} to ${e.max.toFixed(t)}`}function C(e,t,r){return e<t?t:e>r?r:e}function nt(e){return e*Math.PI/180}function me(e){let t=Math.hypot(e[0],e[1],e[2])||1;return[e[0]/t,e[1]/t,e[2]/t]}function La(e,t){return[e[0]-t[0],e[1]-t[1],e[2]-t[2]]}function Lt(e,t){return[e[1]*t[2]-e[2]*t[1],e[2]*t[0]-e[0]*t[2],e[0]*t[1]-e[1]*t[0]]}function Fa(e,t){let r=new Float32Array(16);for(let i=0;i<4;i++)for(let n=0;n<4;n++)r[i*4+n]=e[0+n]*t[i*4+0]+e[4+n]*t[i*4+1]+e[8+n]*t[i*4+2]+e[12+n]*t[i*4+3];return r}function ka(e,t,r,i){let n=1/Math.tan(e*.5),o=1/(r-i),l=new Float32Array(16);return l[0]=n/Math.max(t,1e-6),l[5]=n,l[10]=(i+r)*o,l[11]=-1,l[14]=2*i*r*o,l}function Ia(e,t,r){let i=me(La(e,t)),n=me(Lt(r,i)),o=Lt(i,n),l=new Float32Array(16);return l[0]=n[0],l[1]=o[0],l[2]=i[0],l[3]=0,l[4]=n[1],l[5]=o[1],l[6]=i[1],l[7]=0,l[8]=n[2],l[9]=o[2],l[10]=i[2],l[11]=0,l[12]=-(n[0]*e[0]+n[1]*e[1]+n[2]*e[2]),l[13]=-(o[0]*e[0]+o[1]*e[1]+o[2]*e[2]),l[14]=-(i[0]*e[0]+i[1]*e[1]+i[2]*e[2]),l[15]=1,l}function dr(e){let t=new Float32Array(16),r=e[0],i=e[1],n=e[2],o=e[3],l=e[4],c=e[5],E=e[6],v=e[7],M=e[8],m=e[9],f=e[10],S=e[11],L=e[12],w=e[13],B=e[14],R=e[15],F=r*c-i*l,G=r*E-n*l,W=r*v-o*l,D=i*E-n*c,_=i*v-o*c,z=n*v-o*E,b=M*w-m*L,ae=M*B-f*L,We=M*R-S*L,_e=m*B-f*w,ze=m*R-S*w,Ne=f*R-S*B,g=F*Ne-G*ze+W*_e+D*We-_*ae+z*b;return g?(g=1/g,t[0]=(c*Ne-E*ze+v*_e)*g,t[1]=(n*ze-i*Ne-o*_e)*g,t[2]=(w*z-B*_+R*D)*g,t[3]=(f*_-m*z-S*D)*g,t[4]=(E*We-l*Ne-v*ae)*g,t[5]=(r*Ne-n*We+o*ae)*g,t[6]=(B*W-L*z-R*G)*g,t[7]=(M*z-f*W+S*G)*g,t[8]=(l*ze-c*We+v*b)*g,t[9]=(i*We-r*ze-o*b)*g,t[10]=(L*_-w*W+R*F)*g,t[11]=(m*W-M*_-S*F)*g,t[12]=(c*ae-l*_e-E*b)*g,t[13]=(r*_e-i*ae+n*b)*g,t[14]=(w*G-L*D-B*F)*g,t[15]=(M*D-m*G+f*F)*g,t):null}function Ft(e,t){return[e[0]*t[0]+e[4]*t[1]+e[8]*t[2]+e[12]*t[3],e[1]*t[0]+e[5]*t[1]+e[9]*t[2]+e[13]*t[3],e[2]*t[0]+e[6]*t[1]+e[10]*t[2]+e[14]*t[3],e[3]*t[0]+e[7]*t[1]+e[11]*t[2]+e[15]*t[3]]}var a={sourceImageInfo:null,sourceFile:null,sourceFileName:"",sourceUploadedToWorker:!1,layerSourceFiles:[null,null,null,null],worker:null,workerReady:!1,workerWarming:!1,simReady:!1,simulationLoading:!1,simulationRequestId:0,workerCanvasTransferred:!1,running:!1,lastFrameMs:0,buildCount:0,readbackPending:!1,lastReadbackAt:0,gpuStats:null,sourcePoints:[],isPainting:!1,lastPaintAt:0,cameraPosX:Ba,cameraPosY:Ea,cameraPosZ:Ra,pointerLookActive:!1,navLoopHandle:0,navLastAt:0,navKeys:{KeyW:!1,KeyA:!1,KeyS:!1,KeyD:!1,Space:!1,KeyC:!1},statsFrameHandle:0},ur=1,kt=new Map;function Da(){let e=U.getBoundingClientRect(),t=Math.max(1,Math.round(e.width||960)),r=Math.max(1,Math.round(e.height||Math.round(t*2/3))),i=Math.max(1,Math.min(window.devicePixelRatio||1,2));return{width:t,height:r,dpr:i}}function cr(e){e.addEventListener("message",t=>{let r=t.data||{};if(r.type==="response"&&r.requestId){let i=kt.get(r.requestId);if(!i)return;kt.delete(r.requestId),r.ok?i.resolve(r):i.reject(new Error(r.error||"Worker request failed."));return}if(r.type==="status"){r.stats&&(a.gpuStats=r.stats),Array.isArray(r.sourcePoints)&&(a.sourcePoints=r.sourcePoints),r.sourceImageInfo&&(a.sourceImageInfo=r.sourceImageInfo),Number.isFinite(r.lastFrameMs)&&(a.lastFrameMs=r.lastFrameMs),typeof r.running=="boolean"&&(a.running=r.running),typeof r.gpuReady=="boolean"&&(a.workerWarming=!r.gpuReady),typeof r.simReady=="boolean"&&(a.simReady=r.simReady),p(),T();return}r.type==="workerError"&&(mt("workerError",r.message),p(r.message||"Worker error"))}),e.addEventListener("error",t=>{console.error("[WebGPU Erosion UI] worker error",t),p(t.message||"Worker crashed")})}async function Oa(){if(a.worker)return a.worker;if(!("transferControlToOffscreen"in u))throw new Error("OffscreenCanvas worker rendering is not available in this browser.");let e=new Worker(Pa,{type:"module"});cr(e),a.worker=e;let t=u.transferControlToOffscreen();a.workerCanvasTransferred=!0;let r=Da(),i=await k("init",{canvas:t,width:r.width,height:r.height,dpr:r.dpr},[t]);return a.workerReady=!0,a.workerWarming=!!i.warming,p(),e}function x(e,t={},r=[]){a.worker&&a.worker.postMessage({type:e,...t},r)}function k(e,t={},r=[]){return new Promise((i,n)=>{if(!a.worker){n(new Error("Worker is not initialized."));return}let o=ur++;kt.set(o,{resolve:i,reject:n}),a.worker.postMessage({type:e,requestId:o,...t},r)})}function Ua(){if(!a.workerReady)return;let e=Da();x("resize",e)}function hr(){let e=ee[0],t=C(s(e.hardnessMinLabel,re),0,1),r=C(s(e.hardnessMaxLabel,re+ot),0,1),i=Math.min(t,r),n=Math.max(t,r);return{timeStep:s(Ve,Ot),rainRate:Fe.input.checked?s(ve,Ut):0,evaporationRate:s(Ye,Gt),pipeArea:s($e,Wt),gravity:s(qe,_t),capacityScale:s(Se,zt),suspensionRate:s(we,Nt),depositionRate:s(Pe,Ht),softeningRate:s(Me,Vt),maxErosionDepth:s(Be,Yt),thermalRate:ke.input.checked?s(Ee,$t):0,hydraulicErosionEnabled:!!Ze.input.checked,talusSlopeCoeff:s(Re,qt),talusSlopeBias:s(Ce,Xt),renderHeightScale:s(yt,Kt),waterHeightScale:s(Xe,st),edgeWaterFloor:Math.max(0,s(la,Zt)),waterOpacity:s(xt,jt),sedimentTint:s(bt,Jt),hardnessBase:i,hardnessVariation:Math.max(0,n-i),sourceCenterX:s(da,Qt)/100,sourceCenterY:s(ua,ea)/100,sourceRadius:s(ue,ta),sourceStrength:s(ce,aa),rainDuration:s(Te,lt),pulse2Duration:s(Ke,dt),sourceLayoutMode:Number(Ae.input.value)||0,randomSpringCount:Math.max(1,Math.floor(s(he,ut))),sourceSeed:Math.floor(s(Le,ra)),metersPerPixel:Math.max(1,s(vt,ct)),hydraulic8Way:!!St.input.checked,sourceEnabled:!!ie.input.checked,renderMode:Number(ge.input.value)||0,cameraAzimuthDeg:s(ye,ht),cameraElevationDeg:s(xe,pt),cameraPosX:a.cameraPosX,cameraPosY:a.cameraPosY,cameraPosZ:a.cameraPosZ}}function be(){return oa.input.value||Ta}function pr(){return{mode:be(),layers:ee.map((e,t)=>({enabled:!!e.enableLabel.input.checked,preset:e.materialPresetLabel.input.value,label:`Layer ${t+1}`,heightMin:s(e.heightMinLabel,At[t].heightMin),heightMax:s(e.heightMaxLabel,At[t].heightMax),hardnessMin:C(s(e.hardnessMinLabel,fe.custom.hardnessMin),0,1),hardnessMax:C(s(e.hardnessMaxLabel,fe.custom.hardnessMax),0,1),thermalEnabled:!!e.thermalEnableLabel.input.checked}))}}function q(){return be()==="stack4"?a.layerSourceFiles.some(Boolean):!!a.sourceFile}function Ga(){return be()==="stack4"?a.layerSourceFiles.map((e,t)=>e?`L${t+1}:${e.name}`:null).filter(Boolean).join(" | "):a.sourceFile?.name||""}function mr(e){let t=fe[e.materialPresetLabel.input.value];!t||e.materialPresetLabel.input.value==="custom"||(e.hardnessMinLabel.input.value=String(t.hardnessMin),e.hardnessMaxLabel.input.value=String(t.hardnessMax))}function K(){let e=ee[0],t=s(e.heightMinLabel,It),r=s(e.heightMaxLabel,Dt),i=C(s(e.hardnessMinLabel,re),0,1),n=C(s(e.hardnessMaxLabel,re+ot),0,1),o=Math.min(i,n),l=Math.max(i,n);_a.input.value=String(t),za.input.value=String(r),fr.input.value=String(o),gr.input.value=String(Math.max(0,l-o))}function Wa(){let e=be(),t=e==="stack4",r=e==="single";de.style.display=t?"none":"inline-flex",Ya.textContent=r?"Single grayscale uses only Layer 1. Use that row to set the height range and erodability for the base DEM.":"Use a packed RGBA DEM where each channel is a layer, or a stack of up to four grayscale PNGs. Height min/max defines each layer top surface, erodability min/max defines how easily each layer cuts, and each layer can opt in or out of thermal collapse.";for(let[i,n]of ee.entries()){let o=!r||i===0;n.row.style.display=o?"grid":"none",n.filePickerLabel.style.display=o&&t?"inline-flex":"none",n.filePickerLabel.input.disabled=!(o&&t),n.enableLabel.style.display=r&&i===0?"none":"inline-flex",r&&i===0?(n.enableLabel.input.checked=!0,n.heading.textContent="Layer 1 / base material"):n.heading.textContent=`Layer ${i+1}`}K()}var Q=document.createElement("div");Q.style.boxSizing="border-box";Q.style.padding="16px";Q.style.fontFamily="system-ui, sans-serif";Q.style.color="#e8e8e8";Q.style.background="#111";Q.style.minHeight="100vh";var ft=document.createElement("h2");ft.textContent="WebGPU Fast Hydraulic + Thermal Erosion (Jako)";ft.style.margin="0 0 12px 0";ft.style.fontWeight="600";var le=document.createElement("div");le.style.display="flex";le.style.flexWrap="wrap";le.style.alignItems="center";le.style.gap="8px";le.style.marginBottom="14px";var de=document.createElement("label");de.textContent="DEM PNG";de.style.display="inline-flex";de.style.alignItems="center";de.style.gap="8px";var O=document.createElement("input");O.type="file";O.accept=".png,image/png";O.style.padding="6px 8px";O.style.border="1px solid #444";O.style.borderRadius="6px";O.style.background="#1a1a1a";O.style.color="#e8e8e8";de.appendChild(O);var oa=oe("DEM source",[{value:"single",label:"single grayscale"},{value:"packed_rgba",label:"packed RGBA layers"},{value:"stack4",label:"4 grayscale stack"}],Ta),_a=d("Height floor",It,"88px"),za=d("Height ceiling",Dt,"88px"),Z=d("Sim steps/frame",se,"112px",{min:1,step:1}),gt=d("Single-step size",it,"118px",{min:1,step:1}),Ve=d("Time step",Ot,"88px",{min:.001,step:.001}),ve=d("Background rain",Ut,"108px",{min:0,step:1e-4}),Ye=d("Evaporation",Gt,"96px",{min:0,step:.001}),$e=d("Water flow width",Wt,"112px",{min:.1,step:.1}),qe=d("Gravity",_t,"84px",{min:.1,step:.1}),Se=d("Sediment carry",zt,"104px",{min:0,step:.01}),we=d("Erode into water",Nt,"112px",{min:0,step:.01}),Pe=d("Deposit from water",Ht,"118px",{min:0,step:.01}),Me=d("Bank softening",Vt,"108px",{min:0,step:.01}),Be=d("Max cut depth",Yt,"104px",{min:.001,step:.01}),Ee=d("Slope collapse",$t,"104px",{min:0,step:.01}),Re=d("Slope limit scale",qt,"118px",{min:0,step:.01}),Ce=d("Slope limit bias",Xt,"110px",{min:0,step:.01}),yt=d("Terrain exaggeration",Kt,"130px",{min:0,step:.05}),Xe=d("Water height scale",st,"122px",{min:0,step:.01}),la=d("Min edge water",Zt,"110px",{min:0,step:.005}),xt=d("Water visibility",jt,"112px",{min:0,max:1,step:.05}),bt=d("Sediment boost",Jt,"104px",{min:0,max:2,step:.05}),fr=d("Base erodability",re,"108px",{min:0,max:1,step:.01}),gr=d("Erodability variation",ot,"122px",{min:0,max:1,step:.01}),ie=ne("Enable springs",!1),da=d("Spring X %",Qt,"92px",{min:0,max:100,step:1}),ua=d("Spring Y %",ea,"92px",{min:0,max:100,step:1}),ue=d("Spring radius px",ta,"108px",{min:1,step:1}),ce=d("Spring flow rate",aa,"104px",{min:0,step:.005}),Te=d("Rain duration s",lt,"104px",{min:0,step:.5}),Ke=d("Pulse 2 sec",dt,"92px",{min:0,step:.5}),Ae=oe("Spring mode",[{value:0,label:"painted springs"},{value:1,label:"fixed random springs"}],Ma),he=d("Spring count",ut,"96px",{min:1,max:16,step:1}),Le=d("Spring seed",ra,"96px",{min:0,step:1}),vt=d("Meters / pixel",ct,"104px",{min:1,step:1}),ca=oe("Sim tess",[{value:1,label:"1x"},{value:2,label:"2x"},{value:4,label:"4x"}],Ca),St=ne("8-way hydraulic pipes",rr),Fe=ne("Enable precipitation",nr),ke=ne("Enable thermal erosion",ir),Ze=ne("Enable hydraulic erosion",sr),ge=oe("View",[{value:0,label:"shaded"},{value:2,label:"water"},{value:3,label:"sediment"},{value:4,label:"erodability"},{value:5,label:"thermal"},{value:6,label:"erosion/deposition history"},{value:7,label:"natural geomorph"}],0),Na=oe("Preset",[{value:"paper_balanced",label:"paper balanced"},{value:"river_cut",label:"river cut"},{value:"delta_depositor",label:"delta depositor"},{value:"meander_builder",label:"meander builder"},{value:"thermal_heavy",label:"thermal heavy"},{value:"gentle_weathering",label:"gentle weathering"},{value:"rapid_incision",label:"rapid incision"},{value:"flash_flood",label:"flash flood"},{value:"badlands",label:"badlands"},{value:"canyon_carver",label:"canyon carver"}],"paper_balanced"),ye=d("Azimuth",ht,"84px",{min:-180,max:180,step:1}),xe=d("Elevation",pt,"84px",{min:5,max:89,step:1}),I=oe("Paint",[{value:"none",label:"off"},{value:"raise",label:"terrain +"},{value:"lower",label:"terrain -"},{value:"spring_add",label:"spring +"},{value:"spring_erase",label:"spring erase"}],"none"),wt=d("Brush radius px",na,"112px",{min:1,step:1}),Pt=d("Brush amount",ia,"104px",{min:.001,step:.005}),Mt=d("Brush hardness",sa,"116px",{min:.05,max:1,step:.05}),yr=Object.entries(fe).map(([e,t])=>({value:e,label:t.label})),ee=At.map((e,t)=>{let r=fe[e.preset]||fe.custom,i=document.createElement("div");i.style.display="grid",i.style.gridTemplateColumns="repeat(2, minmax(0, 1fr))",i.style.gap="8px",i.style.padding="8px",i.style.border="1px solid #2c2c2c",i.style.borderRadius="8px",i.style.background="#121212";let n=document.createElement("div");n.textContent=`Layer ${t+1}`,n.style.gridColumn="1 / -1",n.style.fontSize="12px",n.style.fontWeight="600",n.style.opacity="0.95";let o=ne("Enable layer",e.enabled),l=oe("Preset",yr,e.preset),c=d("Height min",e.heightMin,"86px",{min:-10,max:10,step:.01}),E=d("Height max",e.heightMax,"86px",{min:-10,max:10,step:.01}),v=d("Erod min",r.hardnessMin,"86px",{min:0,max:1,step:.01}),M=d("Erod max",r.hardnessMax,"86px",{min:0,max:1,step:.01}),m=ne("Thermal on this layer",e.thermalEnabled!==!1),f=lr(`Layer ${t+1} PNG`);return f.style.gridColumn="1 / -1",i.append(n,o,l,c,E,v,M,m,f),{row:i,heading:n,enableLabel:o,materialPresetLabel:l,heightMinLabel:c,heightMaxLabel:E,hardnessMinLabel:v,hardnessMaxLabel:M,thermalEnableLabel:m,filePickerLabel:f}}),pe=J("Run");pe.disabled=!0;var je=J("Step");je.disabled=!0;var Ha=J("Apply preset"),Bt=J("Reset rain timer");Bt.disabled=!0;var Ie=J("Reset DEM");Ie.disabled=!0;var Va=J("Clear"),N=J("Export DEM PNG");N.disabled=!0;var De=J("Clear painted springs");De.disabled=!0;var Je=document.createElement("div");Je.style.marginLeft="12px";Je.style.opacity="0.9";Je.style.fontSize="14px";le.append(de,Na,Ha,pe,je,N,Ie,Va,Je);var Oe=document.createElement("div");Oe.style.display="grid";Oe.style.gridTemplateColumns="minmax(320px, 1fr) minmax(280px, 380px)";Oe.style.gap="16px";Oe.style.alignItems="start";var te=document.createElement("div");te.style.background="#181818";te.style.border="1px solid #2d2d2d";te.style.borderRadius="12px";te.style.padding="12px";te.style.boxSizing="border-box";te.style.minWidth="0";var Qe=document.createElement("div");Qe.textContent="Render view:";Qe.style.fontSize="15px";Qe.style.fontWeight="600";Qe.style.marginBottom="8px";var U=document.createElement("div");U.style.width="100%";U.style.overflow="hidden";U.style.border="1px solid #333";U.style.borderRadius="10px";U.style.background="#0d0d0d";U.style.position="relative";U.style.aspectRatio="3 / 1.5";var y=document.createElement("canvas");y.width=960;y.height=640;y.style.display="block";y.style.width="100%";y.style.height="100%";y.style.background="#0d0d0d";y.style.position="absolute";y.style.inset="0";y.style.zIndex="0";var u=document.createElement("canvas");u.width=960;u.height=640;u.style.display="block";u.style.width="100%";u.style.height="100%";u.style.background="#0d0d0d";u.style.position="absolute";u.style.inset="0";u.style.zIndex="1";u.style.visibility="hidden";u.style.touchAction="none";u.tabIndex=0;var h=document.createElement("canvas");h.width=960;h.height=640;h.style.display="block";h.style.width="100%";h.style.height="100%";h.style.position="absolute";h.style.inset="0";h.style.zIndex="2";h.style.pointerEvents="none";h.style.visibility="hidden";U.append(y,u,h);te.append(Qe,U);var $=document.createElement("div");$.style.background="#181818";$.style.border="1px solid #2d2d2d";$.style.borderRadius="12px";$.style.padding="12px";$.style.boxSizing="border-box";$.style.maxHeight="85vh";$.style.overflowY="scroll";var et=document.createElement("div");et.textContent="Stats";et.style.fontSize="15px";et.style.fontWeight="600";et.style.marginBottom="8px";var j=document.createElement("div");j.style.whiteSpace="pre-line";j.style.fontSize="13px";j.style.opacity="0.95";j.style.lineHeight="1.45";j.textContent="Load a DEM PNG to begin.";var tt=document.createElement("div");tt.style.display="grid";tt.style.gap="10px";tt.style.marginBottom="12px";var ha=V("Process toggles");ha.append(Y("Master on/off switches for precipitation, springs, hydraulic erosion, and thermal erosion."));ha.body.append(Fe,ie,ke,Ze,Te,Bt);var Et=V("DEM source and layered materials"),Ya=Y("Use a single grayscale DEM, a packed RGBA DEM where each channel is a layer, or a stack of up to four grayscale PNGs. Height min/max defines each layer top surface, erodability min/max defines how easily each layer cuts, and each layer can opt in or out of thermal collapse.");Et.append(Ya);Et.body.append(oa);for(let e of ee)Et.body.append(e.row);var pa=V("Shared water flow and simulation");pa.append(Y("These affect precipitation, springs, water flow, edge runoff retention, and general simulation pacing. They are not thermal-only."));pa.body.append(Z,gt,Ve,ve,Ye,$e,qe,ca,St,vt,la);var ma=V("Hydraulic erosion only");ma.append(Y("These only change hydraulic erosion and deposition behavior. Water can still animate with hydraulic erosion disabled."));ma.body.append(Se,we,Pe,Me,Be);var fa=V("Thermal erosion only");fa.append(Y("These only affect slope collapse and talus behavior."));fa.body.append(Ee,Re,Ce);var ga=V("Springs");ga.append(Y("Spring mode and spring-source controls. Random-spring controls only apply in fixed random springs mode."));ga.body.append(Ae,ue,ce,Ke,he,Le,De);var ya=V("View and shading");ya.append(Y("Render-only controls. These do not change erosion behavior. Natural geomorph uses erosion/deposition history plus thermal activity to tint rock, soil, alluvium, and colluvium."));ya.body.append(ge,yt,Xe,xt,bt);var xa=V("Camera and navigation");xa.append(Y("Orbit inputs below. Click the canvas with paint off for pointer look, then use WASD to move, Space to go up, C to go down, and Esc to release."));xa.body.append(ye,xe);var ba=V("Painting");ba.append(Y("Terrain and spring painting tools. Pointer look is disabled while a paint mode is active."));ba.body.append(I,wt,Pt,Mt);tt.append(xa,ya,Et,ha,pa,ma,fa,ga,ba);$.append(tt,et,j);Oe.append(te,$);Q.append(ft,le,Oe);document.body.style.margin="0";document.body.style.background="#111";document.body.appendChild(Q);window.addEventListener("beforeunload",()=>{Ge(),a.worker&&(x("destroy"),a.worker.terminate(),a.worker=null,a.workerReady=!1,a.workerWarming=!1,a.simReady=!1,a.simulationLoading=!1)});window.addEventListener("resize",()=>{Rt(),Ua(),x("render")});O.addEventListener("change",()=>{let e=O.files?.[0];e&&Mr(e)});oa.input.addEventListener("change",()=>{a.sourceUploadedToWorker=!1,Wa(),K(),q()?X():T()});for(let[e,t]of ee.entries()){t.materialPresetLabel.input.addEventListener("change",()=>{mr(t),e===0&&K(),q()?X():T()});for(let r of[t.enableLabel,t.heightMinLabel,t.heightMaxLabel,t.hardnessMinLabel,t.hardnessMaxLabel,t.thermalEnableLabel])r.input.addEventListener("input",()=>{e===0&&K(),q()?X():P()}),r.input.addEventListener("change",()=>{e===0&&K(),q()?X():P()});t.filePickerLabel.input.addEventListener("change",()=>{let r=t.filePickerLabel.input.files?.[0]||null;a.layerSourceFiles[e]=r,a.sourceUploadedToWorker=!1,a.sourceFileName=Ga(),q()?X():T()})}Ha.addEventListener("click",()=>{Ja(Na.input.value)});pe.addEventListener("click",()=>{a.running?Ge():Cr()});Bt.addEventListener("click",async()=>{if(!a.workerReady||!a.gpuStats?.ready)return;A();let e=await k("resetRainTimer");e.stats&&(a.gpuStats=e.stats),Array.isArray(e.sourcePoints)&&(a.sourcePoints=e.sourcePoints),p("Rain timer reset"),P()});je.addEventListener("click",async()=>{if(!a.workerReady||!a.gpuStats?.ready)return;A();let e=await k("step",{iterations:Math.max(1,Math.floor(s(gt,it))),render:!0});e.stats&&(a.gpuStats=e.stats),Array.isArray(e.sourcePoints)&&(a.sourcePoints=e.sourcePoints),await Tt(!0),P()});Z.input.addEventListener("input",async()=>{let e=Math.max(1,Math.floor(s(Z,se)));P(),a.workerReady&&a.running&&await k("startLoop",{iterationsPerFrame:e})});N.addEventListener("click",async()=>{if(!(!a.workerReady||!a.gpuStats?.ready)){N.disabled=!0,p("Exporting DEM PNG\u2026");try{let e=await k("exportTerrainPng"),t=e.data instanceof ArrayBuffer?e.data:e.data?.buffer;if(!(t instanceof ArrayBuffer))throw new Error("Terrain export did not return PNG data.");let r=new Blob([t],{type:"image/png"}),i=(a.sourceFileName||"terrain").replace(/\.[^.]+$/,""),n=document.createElement("a"),o=URL.createObjectURL(r);n.href=o,n.download=`${i}_dem_export.png`,document.body.appendChild(n),n.click(),n.remove(),setTimeout(()=>URL.revokeObjectURL(o),0);let l=Number.isFinite(e.minTerrain)?e.minTerrain.toFixed(4):"n/a",c=Number.isFinite(e.maxTerrain)?e.maxTerrain.toFixed(4):"n/a";p(`Exported DEM PNG (${e.width}x${e.height}, range ${l} to ${c})`)}catch(e){p(e instanceof Error?e.message:String(e))}finally{N.disabled=!a.gpuStats?.ready}}});De.addEventListener("click",async()=>{if(!a.workerReady)return;let e=await k("clearPaintedSprings");e.stats&&(a.gpuStats=e.stats),Array.isArray(e.sourcePoints)&&(a.sourcePoints=e.sourcePoints),p("Cleared painted springs"),P()});I.input.addEventListener("change",()=>{at(),P()});for(let e of[Fe,ke,Ze])e.input.addEventListener("change",()=>{Ue(),A(),x("render"),P()});for(let e of[wt,Pt,Mt])e.input.addEventListener("input",()=>{P()});Ie.addEventListener("click",()=>{X()});Va.addEventListener("click",wa);var xr=new Set([ue,ce,he,Le]);for(let e of[Ve,ve,Ye,$e,qe,Se,we,Pe,Me,Be,Ee,Re,Ce,yt,Xe,xt,bt,ye,xe,da,ua,ue,ce,Te,Ke,he,Le])e.input.addEventListener("input",()=>{A(),a.running?x("startLoop",{iterationsPerFrame:Math.max(1,Math.floor(s(Z,se)))}):x("render"),xr.has(e)&&Ct(),T()});ie.input.addEventListener("change",()=>{Ue(),K(),A(),x("render"),Ct(),P()});Ae.input.addEventListener("change",()=>{Ue(),A(),x("render"),Ct(),P()});St.input.addEventListener("change",()=>{A(),a.running?x("startLoop",{iterationsPerFrame:Math.max(1,Math.floor(s(Z,se)))}):x("render"),P()});ge.input.addEventListener("change",()=>{A(),x("render"),P()});ca.input.addEventListener("change",()=>{T(),q()&&X()});Ja("paper_balanced");Wa();K();Rt();He(!1);p();Ue();at();H();function $a(){return Number(Ae.input.value)||0}function br(){return $a()===1?"fixed random springs":"painted springs"}function qa(){return I.input.value==="none"}function at(){let e=I.input.value!=="none";document.pointerLockElement===u&&e&&document.exitPointerLock?.(),u.style.cursor=e?"crosshair":document.pointerLockElement===u?"none":"grab"}function vr(){return[a.cameraPosX,a.cameraPosY,a.cameraPosZ]}function va(){let e=nt(s(ye,ht)),t=nt(s(xe,pt)),r=vr(),i=me([-Math.cos(t)*Math.cos(e),-Math.sin(t),-Math.cos(t)*Math.sin(e)]),n=me([i[0],0,i[2]]),o=me(Lt(n,[0,1,0])),l=[r[0]+i[0],r[1]+i[1],r[2]+i[2]];return{azimuth:e,elevation:t,eye:r,target:l,forward:i,forwardFlat:n,right:o}}function Sr(){return!!(a.navKeys.KeyW||a.navKeys.KeyA||a.navKeys.KeyS||a.navKeys.KeyD||a.navKeys.Space||a.navKeys.KeyC)}function Xa(e=performance.now()){if(!a.pointerLookActive){a.navLoopHandle=0,a.navLastAt=0;return}let t=a.navLastAt>0?Math.min(.05,Math.max(.001,(e-a.navLastAt)/1e3)):1/60;if(a.navLastAt=e,Sr()){let r=va(),i=tr*t,n=0,o=0,l=0;a.navKeys.KeyW&&(n+=r.forwardFlat[0]*i,l+=r.forwardFlat[2]*i),a.navKeys.KeyS&&(n-=r.forwardFlat[0]*i,l-=r.forwardFlat[2]*i),a.navKeys.KeyD&&(n+=r.right[0]*i,l+=r.right[2]*i),a.navKeys.KeyA&&(n-=r.right[0]*i,l-=r.right[2]*i),a.navKeys.Space&&(o+=i),a.navKeys.KeyC&&(o-=i),a.cameraPosX+=n,a.cameraPosY+=o,a.cameraPosZ+=l,A(),x("render"),H(),P()}a.pointerLookActive?a.navLoopHandle=requestAnimationFrame(Xa):a.navLoopHandle=0}function Ka(){!a.pointerLookActive||a.navLoopHandle||(a.navLastAt=0,a.navLoopHandle=requestAnimationFrame(Xa))}function wr(){if(a.pointerLookActive=document.pointerLockElement===u,a.pointerLookActive)Ka();else{for(let e of Object.keys(a.navKeys))a.navKeys[e]=!1;a.navLoopHandle&&cancelAnimationFrame(a.navLoopHandle),a.navLoopHandle=0,a.navLastAt=0}at()}function Pr(e){if(document.pointerLockElement!==u||!qa())return;e.preventDefault();let t=ar,r=s(ye,ht)+e.movementX*t,i=C(s(xe,pt)+e.movementY*t,5,89);ye.input.value=String(r),xe.input.value=String(i),A(),x("render"),H(),T()}function Za(e,t){document.pointerLockElement===u&&e.code in a.navKeys&&(e.preventDefault(),a.navKeys[e.code]=t,t&&Ka())}document.addEventListener("pointerlockchange",wr);document.addEventListener("pointermove",Pr,{passive:!1});document.addEventListener("keydown",e=>Za(e,!0),{passive:!1});document.addEventListener("keyup",e=>Za(e,!1),{passive:!1});u.addEventListener("wheel",e=>{document.pointerLockElement===u&&e.preventDefault()},{passive:!1});function Ue(){let e=!!ie.input.checked,t=!!Fe.input.checked,r=!!ke.input.checked,i=!!Ze.input.checked,n=$a()===1;ve.input.disabled=!t,Se.input.disabled=!i,we.input.disabled=!i,Pe.input.disabled=!i,Me.input.disabled=!i,Be.input.disabled=!i,Ee.input.disabled=!r,Re.input.disabled=!r,Ce.input.disabled=!r,Te.input.disabled=!t,Ae.input.disabled=!e,ue.input.disabled=!e||!n,ce.input.disabled=!e||!n,he.input.disabled=!e||!n,Le.input.disabled=!e||!n,Bt.disabled=!a.gpuStats?.ready,at()}u.addEventListener("contextmenu",e=>{(I.input.value!=="none"||document.pointerLockElement===u)&&e.preventDefault()});u.addEventListener("click",e=>{!a.gpuStats?.ready||!qa()||(e.preventDefault(),u.focus(),document.pointerLockElement!==u&&u.requestPointerLock?.())});u.addEventListener("pointerdown",e=>{(I.input.value!=="none"||document.pointerLockElement===u)&&a.gpuStats?.ready&&e.preventDefault(),!(I.input.value==="none"||!a.gpuStats?.ready)&&(a.isPainting=!0,u.setPointerCapture?.(e.pointerId),ja(e))});u.addEventListener("pointermove",e=>{a.isPainting&&(e.preventDefault(),ja(e))});u.addEventListener("pointerup",Sa);u.addEventListener("pointerleave",Sa);u.addEventListener("pointercancel",Sa);function He(e){u.style.visibility=e?"visible":"hidden",h.style.visibility=e?"visible":"hidden",y.style.visibility=e?"hidden":"visible",H(),at()}function Rt(){let e=U.getBoundingClientRect(),t=Math.max(1,Math.round(e.width||960)),r=Math.max(1,Math.round(e.height||Math.round(t*2/3))),i=Math.max(1,Math.min(window.devicePixelRatio||1,2)),n=Math.max(1,Math.round(t*i)),o=Math.max(1,Math.round(r*i));(y.width!==n||y.height!==o)&&(y.width=n,y.height=o),(h.width!==n||h.height!==o)&&(h.width=n,h.height=o),H()}async function Mr(e){mt("loadSourceImage begin",e.name),Ge(),He(!1),Rt(),a.sourceFile=e,a.sourceFileName=e.name,a.sourceImageInfo=null,a.sourceUploadedToWorker=!1,a.gpuStats=null,a.sourcePoints=[],a.lastFrameMs=0,a.simReady=!1,a.simulationLoading=!1,Ie.disabled=!1,N.disabled=!0,De.disabled=!0,p("PNG selected, initializing GPU\u2026"),T();try{await X()}catch(t){wa(),Oa().catch(r=>{console.error("[WebGPU Erosion UI] worker prewarm failed",r),p(r instanceof Error?r.message:String(r))}),p(t instanceof Error?t.message:String(t))}}async function X(){if(!q())return;let e=++a.simulationRequestId;Ge(),N.disabled=!0,a.simulationLoading=!0,a.simReady=!1,He(!1),p("Loading DEM\u2026");try{if(Rt(),await Oa(),e!==a.simulationRequestId)return;Ua(),K(),A();let t=be(),r={demSourceMode:t,tessellation:Aa(),options:{minHeight:s(_a,It),maxHeight:s(za,Dt),demSourceMode:t,layerConfig:pr()}};a.sourceUploadedToWorker||(t==="stack4"?r.layerBlobs=a.layerSourceFiles.slice():r.blob=a.sourceFile);let i=await k("loadDEMImage",r);if(e!==a.simulationRequestId)return;if(a.sourceUploadedToWorker=!0,a.buildCount++,a.sourceFileName=Ga(),a.sourceImageInfo=i.sourceImageInfo??a.sourceImageInfo,a.gpuStats=i.stats??a.gpuStats,a.sourcePoints=i.sourcePoints??a.sourcePoints,a.simReady=!!(i.stats?.ready||i.ready),pe.disabled=!1,je.disabled=!1,N.disabled=!1,De.disabled=!1,Ie.disabled=!1,Ue(),He(a.simReady),a.simReady){if(await k("render",{waitForCompletion:!0}),e!==a.simulationRequestId)return;p("DEM loaded"),Tt(!0)}else p("DEM uploaded, waiting for first render\u2026");T()}catch(t){if(e!==a.simulationRequestId)return;console.error("[WebGPU Erosion UI] initializeSimulation failed",t),a.simReady=!1,He(!1),p(t instanceof Error?t.message:String(t))}finally{e===a.simulationRequestId&&(a.simulationLoading=!1,p())}}function P(){a.statsFrameHandle||(a.statsFrameHandle=requestAnimationFrame(()=>{a.statsFrameHandle=0,T()}))}function A(){let e=hr();a.workerReady&&x("setParams",{params:e})}async function Ct(){if(a.workerReady)try{let e=await k("getStatus");e.stats&&(a.gpuStats=e.stats),Array.isArray(e.sourcePoints)&&(a.sourcePoints=e.sourcePoints),typeof e.running=="boolean"&&(a.running=e.running),Number.isFinite(e.lastFrameMs)&&(a.lastFrameMs=e.lastFrameMs),H(),P()}catch(e){mt("refreshWorkerStatusLight failed",e)}}function Br(){return I.input.value==="spring_add"||I.input.value==="spring_erase"}function Er(){return{mode:I.input.value,radius:Math.max(1,s(wt,na)),amount:Math.max(.001,s(Pt,ia)),hardness:C(s(Mt,sa),.05,1)}}function Rr(e){let t=a.gpuStats;if(!t?.ready||!t.width||!t.height)return null;let r=u.getBoundingClientRect(),i=(e.clientX-r.left)/Math.max(r.width,1),n=(e.clientY-r.top)/Math.max(r.height,1);if(i<0||i>1||n<0||n>1)return null;let o=va(),l=t.width>1||t.height>1?2/Math.max(t.width-1,t.height-1,1):1,c=Ia(o.eye,o.target,[0,1,0]),E=ka(nt(50),Math.max(r.width/Math.max(r.height,1),1e-6),.01,32),v=dr(Fa(E,c));if(!v)return null;let M=i*2-1,m=1-n*2,f=Ft(v,[M,m,-1,1]),S=Ft(v,[M,m,1,1]),L=[f[0]/f[3],f[1]/f[3],f[2]/f[3]],w=[S[0]/S[3],S[1]/S[3],S[2]/S[3]],B=me(La(w,L)),R,F,G=0;if(Math.abs(B[1])>1e-5){let b=(G-L[1])/B[1];b>0&&(R=L[0]+B[0]*b,F=L[2]+B[2]*b)}if(!Number.isFinite(R)||!Number.isFinite(F)){let b=i*Math.max(t.width-1,0),ae=n*Math.max(t.height-1,0);return{x:C(b,0,Math.max(t.width-1,0)),y:C(ae,0,Math.max(t.height-1,0))}}let W=(t.width-1)*.5,D=(t.height-1)*.5,_=R/Math.max(l,1e-6)+W,z=D-F/Math.max(l,1e-6);return{x:C(_,0,Math.max(t.width-1,0)),y:C(z,0,Math.max(t.height-1,0))}}async function ja(e){let t=Er();if(t.mode==="none"||!a.workerReady||!a.gpuStats?.ready)return;let r=performance.now();if(r-a.lastPaintAt<20)return;a.lastPaintAt=r;let i=Rr(e);if(i){if(a.running&&Ge(),t.mode==="raise"||t.mode==="lower"){let n=await k("paintTerrainBrush",{brush:{x:i.x,y:i.y,radius:t.radius,amount:t.amount,hardness:t.hardness,subtract:t.mode==="lower"}});n.stats&&(a.gpuStats=n.stats),Array.isArray(n.sourcePoints)&&(a.sourcePoints=n.sourcePoints),p(`${t.mode==="raise"?"Raised":"Lowered"} terrain at ${i.x.toFixed(1)}, ${i.y.toFixed(1)}`)}else{let n=await k("paintSpringBrush",{brush:{x:i.x,y:i.y,radius:t.radius,strength:t.amount,hardness:t.hardness,erase:t.mode==="spring_erase"}});n.stats&&(a.gpuStats=n.stats),Array.isArray(n.sourcePoints)&&(a.sourcePoints=n.sourcePoints),p(`${t.mode==="spring_erase"?"Erased":"Painted"} spring at ${i.x.toFixed(1)}, ${i.y.toFixed(1)}`)}T()}}function Sa(){a.isPainting&&(a.isPainting=!1,Tt(!0))}function Ja(e){let t={paper_balanced:{iterationsPerFrame:5,stepIterations:128,timeStep:.02,rainRate:.001,evaporationRate:.015,pipeArea:20,gravity:9.81,capacityScale:.82,suspensionRate:.32,depositionRate:1.22,softeningRate:2.8,maxErosionDepth:.09,thermalRate:.24,talusCoeff:.92,talusBias:.12,sourceStrength:.06,sourceRadius:4,sourceEnabled:!1,sourceLayoutMode:0,randomSpringCount:1,rainDuration:0,pulse2Duration:0,metersPerPixel:100,waterHeightScale:.06,edgeWaterFloor:0,renderMode:0},river_cut:{iterationsPerFrame:5,stepIterations:160,timeStep:.02,rainRate:.0015,evaporationRate:.012,pipeArea:22,gravity:9.81,capacityScale:.95,suspensionRate:.48,depositionRate:1.02,softeningRate:2.8,maxErosionDepth:.08,thermalRate:.22,talusCoeff:.88,talusBias:.11,sourceStrength:.09,sourceRadius:4,sourceEnabled:!0,sourceLayoutMode:1,randomSpringCount:4,rainDuration:0,pulse2Duration:0,metersPerPixel:100,waterHeightScale:.06,edgeWaterFloor:0,renderMode:0},delta_depositor:{iterationsPerFrame:5,stepIterations:192,timeStep:.02,rainRate:.016,evaporationRate:.004,pipeArea:30,gravity:9.81,capacityScale:.38,suspensionRate:.12,depositionRate:2.2,softeningRate:1.4,maxErosionDepth:.04,thermalRate:.08,talusCoeff:.82,talusBias:.1,sourceStrength:0,sourceRadius:5,sourceEnabled:!1,sourceLayoutMode:0,randomSpringCount:1,rainDuration:85,pulse2Duration:0,metersPerPixel:100,waterHeightScale:.07,renderMode:0},meander_builder:{iterationsPerFrame:5,stepIterations:160,timeStep:.02,rainRate:.003,evaporationRate:.004,pipeArea:32,gravity:9.81,capacityScale:.3,suspensionRate:.1,depositionRate:2.45,softeningRate:1.1,maxErosionDepth:.035,thermalRate:.18,talusCoeff:.86,talusBias:.1,sourceStrength:.11,sourceRadius:5,sourceEnabled:!0,sourceLayoutMode:1,randomSpringCount:3,rainDuration:0,pulse2Duration:0,metersPerPixel:100,waterHeightScale:.07,renderMode:0},thermal_heavy:{iterationsPerFrame:5,stepIterations:128,timeStep:.02,rainRate:0,evaporationRate:.008,pipeArea:8,gravity:9.81,capacityScale:.1,suspensionRate:.05,depositionRate:.85,softeningRate:1,maxErosionDepth:.06,thermalRate:1.1,talusCoeff:.55,talusBias:.05,sourceStrength:0,sourceRadius:4,sourceEnabled:!1,sourceLayoutMode:0,randomSpringCount:1,rainDuration:0,pulse2Duration:0,metersPerPixel:100,waterHeightScale:.06,renderMode:5},gentle_weathering:{iterationsPerFrame:5,stepIterations:96,timeStep:.02,rainRate:.004,evaporationRate:.015,pipeArea:12,gravity:9.81,capacityScale:.45,suspensionRate:.18,depositionRate:1,softeningRate:2,maxErosionDepth:.1,thermalRate:.12,talusCoeff:.8,talusBias:.1,sourceStrength:0,sourceRadius:4,sourceEnabled:!1,sourceLayoutMode:0,randomSpringCount:1,rainDuration:0,pulse2Duration:0,metersPerPixel:100,waterHeightScale:.06,edgeWaterFloor:0,renderMode:0},rapid_incision:{iterationsPerFrame:5,stepIterations:192,timeStep:.022,rainRate:.018,evaporationRate:.01,pipeArea:26,gravity:9.81,capacityScale:1.4,suspensionRate:.9,depositionRate:.75,softeningRate:6,maxErosionDepth:.1,thermalRate:.12,talusCoeff:.72,talusBias:.08,sourceStrength:0,sourceRadius:4,sourceEnabled:!1,sourceLayoutMode:0,randomSpringCount:1,rainDuration:0,pulse2Duration:0,metersPerPixel:100,waterHeightScale:.06,edgeWaterFloor:0,renderMode:0},flash_flood:{iterationsPerFrame:5,stepIterations:192,timeStep:.02,rainRate:.03,evaporationRate:.008,pipeArea:24,gravity:9.81,capacityScale:1.3,suspensionRate:.85,depositionRate:.8,softeningRate:5,maxErosionDepth:.08,thermalRate:.1,talusCoeff:.75,talusBias:.08,sourceStrength:0,sourceRadius:4,sourceEnabled:!1,sourceLayoutMode:0,randomSpringCount:1,rainDuration:50,pulse2Duration:0,metersPerPixel:100,waterHeightScale:.06,renderMode:0},badlands:{iterationsPerFrame:5,stepIterations:160,timeStep:.02,rainRate:.01,evaporationRate:.012,pipeArea:18,gravity:9.81,capacityScale:1,suspensionRate:.55,depositionRate:.95,softeningRate:4,maxErosionDepth:.09,thermalRate:.22,talusCoeff:.68,talusBias:.08,sourceStrength:0,sourceRadius:4,sourceEnabled:!1,sourceLayoutMode:0,randomSpringCount:1,rainDuration:25,pulse2Duration:0,metersPerPixel:100,waterHeightScale:.06,renderMode:0},canyon_carver:{iterationsPerFrame:5,stepIterations:224,timeStep:.022,rainRate:.014,evaporationRate:.01,pipeArea:28,gravity:9.81,capacityScale:1.55,suspensionRate:1,depositionRate:.7,softeningRate:6,maxErosionDepth:.08,thermalRate:.16,talusCoeff:.72,talusBias:.08,sourceStrength:.08,sourceRadius:3,sourceEnabled:!0,sourceLayoutMode:1,randomSpringCount:4,rainDuration:0,pulse2Duration:0,metersPerPixel:100,waterHeightScale:.06,edgeWaterFloor:0,renderMode:0}},r=t[e]||t.paper_balanced;Z.input.value=String(r.iterationsPerFrame??se),gt.input.value=String(r.stepIterations??it),Ve.input.value=String(r.timeStep),ve.input.value=String(r.rainRate),Ye.input.value=String(r.evaporationRate),$e.input.value=String(r.pipeArea),qe.input.value=String(r.gravity),Se.input.value=String(r.capacityScale),we.input.value=String(r.suspensionRate),Pe.input.value=String(r.depositionRate),Me.input.value=String(r.softeningRate),Be.input.value=String(r.maxErosionDepth),Ee.input.value=String(r.thermalRate),Re.input.value=String(r.talusCoeff),Ce.input.value=String(r.talusBias),ce.input.value=String(r.sourceStrength),Te.input.value=String(r.rainDuration??lt),Ke.input.value=String(r.pulse2Duration??dt),ue.input.value=String(r.sourceRadius),Fe.input.checked=(r.rainRate??0)>0,ke.input.checked=(r.thermalRate??0)>0,Ze.input.checked=!0,ie.input.checked=!!r.sourceEnabled,Ae.input.value=String(r.sourceLayoutMode??Ma),he.input.value=String(r.randomSpringCount??ut),vt.input.value=String(r.metersPerPixel??ct),Xe.input.value=String(r.waterHeightScale??st),la.input.value=String(r.edgeWaterFloor??Zt),ge.input.value=String(r.renderMode),Ue(),A(),x("render"),Ct(),T()}async function Tt(e=!1){if(!a.workerReady||a.readbackPending)return;let t=performance.now();if(!(!e&&t-a.lastReadbackAt<500)){a.readbackPending=!0;try{let r=await k("readbackStats");a.gpuStats=r.stats??a.gpuStats,a.sourcePoints=r.sourcePoints??a.sourcePoints,a.sourceImageInfo=r.sourceImageInfo??a.sourceImageInfo,Number.isFinite(r.lastFrameMs)&&(a.lastFrameMs=r.lastFrameMs),a.lastReadbackAt=performance.now()}catch(r){mt("readback failed",r)}finally{a.readbackPending=!1}}}function Cr(){!a.workerReady||a.running||(a.running=!0,pe.textContent="Pause",A(),x("startLoop",{iterationsPerFrame:Math.max(1,Math.floor(s(Z,se)))}),Tt(!0),p("Running"))}function Ge(){a.running&&(a.running=!1,pe.textContent="Run",x("stopLoop"),p("Paused"))}function wa(){Ge(),document.exitPointerLock?.(),x("clear"),a.sourceImageInfo=null,a.sourceFile=null,a.layerSourceFiles=[null,null,null,null],a.sourceUploadedToWorker=!1,a.buildCount=0,a.lastFrameMs=0,a.readbackPending=!1,a.lastReadbackAt=0,a.gpuStats=null,a.sourcePoints=[],a.sourceFileName="",a.simReady=!1,a.simulationLoading=!1,a.workerWarming=!1,a.cameraPosX=Ba,a.cameraPosY=Ea,a.cameraPosZ=Ra,O.value="";for(let t of ee)t.filePickerLabel.input.value="";pe.disabled=!0,je.disabled=!0,N.disabled=!0,De.disabled=!0,Ie.disabled=!0,p(),T(),H();let e=y.getContext("2d");e&&(e.fillStyle="#0d0d0d",e.fillRect(0,0,y.width,y.height),e.fillStyle="rgba(255,255,255,0.55)",e.font="16px system-ui, sans-serif",e.fillText("Load a DEM PNG, packed RGBA DEM, or 4-layer grayscale stack to initialize the worker-owned WebGPU erosion sim.",24,36))}function H(){let e=h.getContext("2d");if(!e||(e.clearRect(0,0,h.width,h.height),h.style.visibility==="hidden"))return;let t=a.gpuStats,r=a.sourcePoints||[];if(!t?.ready||!t.width||!t.height||r.length===0)return;let i=va(),n=Ia(i.eye,i.target,[0,1,0]),o=ka(nt(50),Math.max(h.width/Math.max(h.height,1),1e-6),.01,32),l=Fa(o,n),c=t.width>1||t.height>1?2/Math.max(t.width-1,t.height-1,1):1,E=(t.width-1)*.5,v=(t.height-1)*.5,M=Math.max(2,Math.min(6,Math.min(h.width,h.height)/Math.max(t.width,t.height,1)*.85));for(let m of r){let f=(m.x-E)*c,S=(v-m.y)*c,w=Ft(l,[f,0,S,1]);if(!w[3]||w[3]<=0)continue;let B=w[0]/w[3],R=w[1]/w[3];if(Math.abs(B)>1.2||Math.abs(R)>1.2)continue;let F=(B*.5+.5)*h.width,G=(1-(R*.5+.5))*h.height,W=!!m.painted,D=m.active!==!1;if(!W&&!Br())continue;if(W){let z=C(Number(m.strength)||0,0,1),b=M*(.8+.9*z);e.fillStyle=D?"rgba(176, 96, 255, 0.98)":"rgba(120, 96, 150, 0.85)",e.fillRect(Math.round(F-b),Math.round(G-b),Math.max(1,Math.round(b*2)),Math.max(1,Math.round(b*2)));continue}let _=Math.max(5,(m.radius||2)*Math.min(h.width,h.height)/Math.max(t.width,t.height,1)*1.2);e.beginPath(),e.arc(F,G,_,0,Math.PI*2),e.fillStyle=D?"rgba(60, 220, 255, 0.30)":"rgba(90, 110, 120, 0.18)",e.fill(),e.lineWidth=2,e.strokeStyle=D?"rgba(138, 240, 255, 0.95)":"rgba(140, 165, 175, 0.55)",e.stroke()}}function p(e=""){let t=[];e&&t.push(e),t.push(a.sourceFile?`image: ${a.sourceFileName||"loaded"}`:"image: none"),a.workerReady?a.workerWarming?t.push("webgpu: warming"):t.push("webgpu: worker ready"):t.push("webgpu: idle"),a.simulationLoading?t.push("dem: loading"):a.simReady?t.push("dem: ready"):(a.sourceFile||a.layerSourceFiles.some(Boolean))&&t.push("dem: not rendered"),a.running&&t.push("sim: running"),Je.textContent=t.join("  |  ")}function T(){if(!q()&&!a.sourceImageInfo){j.textContent="Load a DEM PNG or layered DEM stack to begin.",H();return}let e=a.sourceImageInfo?.width||a.gpuStats?.width||0,t=a.sourceImageInfo?.height||a.gpuStats?.height||0,r=a.gpuStats?.width||e,i=a.gpuStats?.height||t,n=a.gpuStats??{width:0,height:0,cellCount:0,ready:!1,iterationCount:0},o=a.lastFrameMs>0?1e3/Math.max(a.lastFrameMs,1e-6):0,l=[`fps: ${Number.isFinite(o)&&o>0?o.toFixed(1):"n/a"}`,`last frame: ${a.lastFrameMs.toFixed(3)} ms`,`erosion cpu: ${Number.isFinite(n.lastCpuErosionPassMs)?n.lastCpuErosionPassMs.toFixed(3):Number.isFinite(n.lastErosionPassMs)?n.lastErosionPassMs.toFixed(3):"n/a"} ms`,`erosion gpu: ${Number.isFinite(n.lastGpuErosionPassMs)?n.lastGpuErosionPassMs.toFixed(3):"n/a"} ms`,`render cpu: ${Number.isFinite(n.lastCpuRenderPassMs)?n.lastCpuRenderPassMs.toFixed(3):Number.isFinite(n.lastRenderPassMs)?n.lastRenderPassMs.toFixed(3):"n/a"} ms`,`render gpu: ${Number.isFinite(n.lastGpuRenderPassMs)?n.lastGpuRenderPassMs.toFixed(3):"n/a"} ms`,`erosion cpu avg: ${Number.isFinite(n.avgCpuErosionPassMs)?n.avgCpuErosionPassMs.toFixed(3):Number.isFinite(n.avgErosionPassMs)?n.avgErosionPassMs.toFixed(3):"n/a"} ms`,`erosion gpu avg: ${Number.isFinite(n.avgGpuErosionPassMs)?n.avgGpuErosionPassMs.toFixed(3):"n/a"} ms`,`render cpu avg: ${Number.isFinite(n.avgCpuRenderPassMs)?n.avgCpuRenderPassMs.toFixed(3):Number.isFinite(n.avgRenderPassMs)?n.avgRenderPassMs.toFixed(3):"n/a"} ms`,`render gpu avg: ${Number.isFinite(n.avgGpuRenderPassMs)?n.avgGpuRenderPassMs.toFixed(3):"n/a"} ms`,"",`file: ${a.sourceFileName||"(loaded image)"}`,`DEM source: ${be()}`,`layered DEM active: ${n.layeredDemEnabled?"yes":"no"}`,`active layers: ${Number.isFinite(n.activeLayerCount)?n.activeLayerCount:ee.filter(c=>c.enableLabel.input.checked).length}`,`image size: ${e} x ${t}`,`sim tess: ${Aa()}x`,`sim grid: ${n.width||r} x ${n.height||i}`,`cells: ${n.cellCount||r*i}`,`gpu ready: ${n.ready?"yes":"no"}`,`build count: ${a.buildCount}`,`iterations: ${n.iterationCount||0}`,`sim time: ${Number.isFinite(n.simTime)?n.simTime.toFixed(2):"0.00"} s`,`rain active: ${n.rainActive?"yes":"no"}`,`precipitation: ${Fe.input.checked?"on":"off"}`,`spring toggle: ${ie.input.checked?"on":"off"}`,`thermal erosion: ${ke.input.checked?"on":"off"}`,`spring mode: ${br()}`,`painted springs: ${(a.sourcePoints||[]).filter(c=>c.painted).length}`,`random spring count: ${Math.max(1,Math.floor(s(he,ut)))}`,`spring seed: ${Math.floor(s(Le,ra))}`,`spring centers: ${(a.sourcePoints||[]).slice(0,6).map(c=>`${c.painted?"p:":"r:"}${c.x.toFixed(1)},${c.y.toFixed(1)}`).join(" | ")||"n/a"}`,`meters / pixel: ${s(vt,ct).toFixed(0)}`,`running: ${a.running?"yes":"no"}`,`pointer look: ${a.pointerLookActive?"locked":"off"}`,`camera pos: ${a.cameraPosX.toFixed(2)}, ${a.cameraPosY.toFixed(2)}, ${a.cameraPosZ.toFixed(2)}`,`paint mode: ${I.input.options[I.input.selectedIndex]?.textContent||"off"}`,`brush radius: ${s(wt,na).toFixed(1)}`,`brush amount: ${s(Pt,ia).toFixed(3)}`,`brush hardness: ${s(Mt,sa).toFixed(2)}`,`readback pending: ${a.readbackPending?"yes":"no"}`,"",`terrain range: ${rt(n.terrainRange)}`,`water range: ${rt(n.waterRange)}`,`sediment range: ${rt(n.sedimentRange)}`,`history range: ${rt(n.historyRange)}`,`total water: ${Number.isFinite(n.totalWater)?n.totalWater.toFixed(4):"n/a"}`,`total sediment: ${Number.isFinite(n.totalSediment)?n.totalSediment.toFixed(4):"n/a"}`,`avg erodability: ${Number.isFinite(n.averageHardness)?n.averageHardness.toFixed(4):"n/a"}`,"",`\u0394t: ${s(Ve,Ot).toFixed(3)}`,`Kr rain: ${s(ve,Ut).toFixed(4)}`,`Ke evap: ${s(Ye,Gt).toFixed(3)}`,`A pipe: ${s($e,Wt).toFixed(2)}`,`hydraulic pipes: ${St.input.checked?"8-way":"4-way"}`,`g gravity: ${s(qe,_t).toFixed(2)}`,`Kc capacity: ${s(Se,zt).toFixed(3)}`,`Ks suspend: ${s(we,Nt).toFixed(3)}`,`Kd deposit: ${s(Pe,Ht).toFixed(3)}`,`Kh soften: ${s(Me,Vt).toFixed(3)}`,`depth cap: ${s(Be,Yt).toFixed(3)}`,`Kt thermal: ${s(Ee,$t).toFixed(3)}`,`Ka talus: ${s(Re,qt).toFixed(3)}`,`Ki talus bias: ${s(Ce,Xt).toFixed(3)}`,`render h: ${s(yt,Kt).toFixed(3)}`,`water h: ${s(Xe,st).toFixed(3)}`,`water \u03B1: ${s(xt,jt).toFixed(3)}`,`sediment tint: ${s(bt,Jt).toFixed(3)}`,`springs: ${ie.input.checked?"on":"off"}`,`rain duration s: ${s(Te,lt).toFixed(2)}`,`pulse 2 s: ${s(Ke,dt).toFixed(2)}`,`source center: ${s(da,Qt).toFixed(0)}%, ${s(ua,ea).toFixed(0)}%`,`source radius: ${s(ue,ta).toFixed(1)}`,`source strength: ${s(ce,aa).toFixed(3)}`,`view mode: ${ge.input.options[ge.input.selectedIndex]?.textContent||"shaded"}`,`iters/frame: ${Math.max(1,Math.floor(s(Z,se)))}`,`step iters: ${Math.max(1,Math.floor(s(gt,it)))}`];j.textContent=l.join(`
`),p(),H()}wa();})();
